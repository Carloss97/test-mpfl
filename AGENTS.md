...
- **Acceso dev (local)** — `vite dev` sin `VITE_KRUMM_API_BASE`: sin endpoint configurado, el validador local (`localValidateInvitationToken`) acepta cualquier token bien formado — p. ej. `?invite=tok-dev-dev1234` entra a setup → gameplay en modo demo determinista. Añade `&battery=original` para la batería de 6 juegos (incluye BOMB). **Solo en local**: en stage y prod (modo real, con `VITE_KRUMM_API_BASE`) el token se valida contra el backend y uno inválido muestra "invitación no válida".
