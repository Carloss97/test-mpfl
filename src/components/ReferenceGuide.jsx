      ...
      <Section title={t('Demo MVP de postulaciones', 'Applications MVP demo')}>
        ...
        <table className="guide-table">
          ...
          <tbody>
            ...
            <tr><td>B — {t('Setup/señales', 'Setup/signals')}</td><td>PostulationConsentSetup.jsx, BackgroundSignalOrchestrator.jsx, SignalErrorHint.jsx</td><td>{t('Completada', 'Completed')}</td><td>{t('Consentimiento, cámara local opcional, worker facial, MoveNet bajo FPS e indicador discreto de error de señal (H2: sin HUD "detrás").', 'Consent, optional local camera, facial worker, MoveNet at low FPS and a discrete signal-error hint (H2: no behind-the-scenes HUD).')}</td></tr>
            <tr><td>C — {t('Game stage', 'Game stage')}</td><td>PostulationGameStage.jsx, PostulationProgressHeader.jsx, postulationDemoConfig.js</td><td>{t('Completada', 'Completed')}</td><td>{t('Juegos principales en fullscreen con progreso e indicador discreto de error de señal; eventos', 'Main games in fullscreen with progress and a discrete signal-error hint; events')} <code>game_event_v1</code>{t(' con ', ' with ')}<code>performance.now()</code>{t('.', '.')}</td></tr>
