...
// v9.2.0 (T.4/T.5): re-nombra canales faciales/game-aware con alias de compatibilidad.
//   stressResponse -> tensionSignal   (señal de tensión facial, NO estrés)
//   adaptiveResilience -> recoveryContext (contexto de recuperación, NO adaptabilidad)
// Schema de salida edge_ai_model_output_v8 se mantiene (alias a claves viejas).
const MODEL_VERSION = 'krumm-edge-ai-v9.2.0-game-aware';
...
const CHANNEL_LABELS = {
  ...
  tensionSignal: { es: 'Señal de tensión facial', en: 'Facial tension signal' },
  ...
  recoveryContext: { es: 'Contexto de recuperación', en: 'Recovery context' },
};

// Alias de compatibilidad: consumidores antiguos siguen leyendo la clave vieja.
const CHANNEL_DEPRECATED_ALIASES = {
  tensionSignal: 'stressResponse',
  recoveryContext: 'adaptiveResilience',
};
...
const CHANNEL_LIKELIHOODS = {
  ...
  tensionSignal:  { AU4: 2.5, AU23: 4.0, AU9: 2.5, AU7: 2.0, AU15: 1.5, AU6: 0.3, AU12: 0.3 },
...
function scoreRecoveryContext(multimodal) {
  if (!multimodal.game?.available) return channelFromRaw(0.5, { confidence: 0, source: 'unavailable', caveats: ['game_telemetry_unavailable'] });
  ...
  return channelFromRaw(raw, {
    confidence: 0.8,
    source: 'game_telemetry+correlation',
    caveats: ['single_session_recovery_context_not_adaptability'],
  });
}
...
function applyMultimodalChannelModifiers(channels, multimodal) {
  ...
  if (channels.tensionSignal) {
    const score = Math.round(channels.tensionSignal.score * 0.75 + posturePenalty * 0.15 + gazeInstability * 0.10);
    channels.tensionSignal = { ...channels.tensionSignal, score, level: levelForScore(score), multimodalAdjusted: true, caveats: [...(channels.tensionSignal.caveats ?? []), 'facial_tension_not_stress_inference'] };
  }
...
const COMPOSITE_WEIGHTS = {
  ...
  recoveryContext: { weight: 0.08, polarity: 1 },
  ...
  tensionSignal: { weight: 0.06, polarity: -1 },
};
...
export function runEdgeAIInference({
  ...
  const channels = {};
  const explicitChannels = new Set(['taskPerformance', 'visualAttention', 'postureQuality', 'inhibitionControl', 'visuomotorPrecision', 'visualSearchEfficiency', 'recoveryContext']);
  ...
  if (channels.visualSearchEfficiency) {
    channels.visualSearchEfficiency = {
      ...channels.visualSearchEfficiency,
      contextOnly: true,
      caveats: [...(channels.visualSearchEfficiency.caveats ?? []), 'context_only_no_validated_source'],
    };
  }
  channels.recoveryContext = scoreRecoveryContext(multimodal);
  ...
  // Alias de compatibilidad: claves viejas siguen presentes apuntando al canal nuevo.
  for (const [newName, oldName] of Object.entries(CHANNEL_DEPRECATED_ALIASES)) {
    if (labeledChannels[newName] && !labeledChannels[oldName]) {
      labeledChannels[oldName] = { ...labeledChannels[newName], deprecatedAlias: newName };
    }
  }
  ...
  return {
    ...
    emotions: emotions
      ? {
          ...emotions,
          framing: 'expression_facs_class',
          notEmotionState: true,
        }
      : emotions,
  };
}
