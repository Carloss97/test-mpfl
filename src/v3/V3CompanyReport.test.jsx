...
describe('G. Reporte demo ES (motor H4.3)', () => {
  ...
  it('resumen ejecutivo (engine): headline + status + 4 cards', () => {
    ...
    expect(screen.getByText('5/5 juegos completados')).toBeInTheDocument();
    expect(screen.getByText('Cómo usarlo')).toBeInTheDocument();
    expect(screen.getByText('Guía de entrevista')).toBeInTheDocument();
    expect(screen.getByText('8 constructos con señal de prueba')).toBeInTheDocument();
    expect(screen.getByText(/2 lectura\(s\) se mantienen descriptivas/)).toBeInTheDocument();
    expect(screen.getByText('Validar antes de comparar candidatos')).toBeInTheDocument();
  });

  it('mapa de evidencia: 8 constructos (6 provisional + 2 descriptivos) + warning + score provisional', () => {
    renderReport('supervisor', 'maria-gonzalez');
    ...
    // los 2 descriptivos sin score (R-6: decisionMaking + adaptability)
    expect(screen.getAllByText('Descriptivo')).toHaveLength(2);
    expect(screen.getAllByText('Lectura descriptiva')).toHaveLength(2);
  ...
  it('resultados por juego: 5 juegos de la batería original, completados', () => {
    renderReport('supervisor', 'maria-gonzalez');
    expect(screen.getByText('Resultados por juego')).toBeInTheDocument();
    for (const game of ['Puzzle láser', 'Globo de riesgo', 'Optimización de rutas de pasajeros', 'Operación Faro: coordinación de equipo', 'Ensamblaje Geométrico (Tangram)']) {
...
describe('H. Reporte demo EN', () => {
  it('títulos/labels/constructos/juegos en inglés', () => {
    ...
    expect(screen.getByText('5/5 games completed')).toBeInTheDocument();
    ...
    expect(screen.getAllByText('Descriptive')).toHaveLength(2);
