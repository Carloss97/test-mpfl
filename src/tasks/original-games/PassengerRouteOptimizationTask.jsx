...
import { GAME_KEYBOARD, passengerKeyAction } from './gameKeyboard.js';
...
function PassengerRouteInner({ emit, trialCount, width, height, onComplete, practice = false }) {
  const { t } = useLanguage();
  ...
  const [circuitToast, setCircuitToast] = useState(null);
  ...
  useEffect(() => {
    if (!introDone || !level || finished || failed || shownLevelsRef.current.has(level.id)) return;
  ...
  }, [failed, finished, introDone, level, levelIndex, levelSolutions, levels.length]);
  ...
  const completeLevel = useCallback(() => {
    ...
    if (isLastLevel) {
      ...
      onCompleteRef.current?.({
        ...
        ...(practice ? markPracticeSummary('passenger_routes', aggregate) : aggregate),
      });
      return;
    }
    ...
    setCircuitToast(`Circuito ${levelIndex + 1} completado`);
    toastTimeoutRef.current = window.setTimeout(() => setCircuitToast(null), 1400);
  ...
  }, [buildAggregate, finished, level, levelIndex, levelSolutions, levels, practice]);
  ...
  const move = useCallback((directionName) => {
    if (!introDone || !level || finished) return;
    ...
    const stationUsed = Boolean(station && nextBudget < level.routeBudget);
    if (stationUsed) {
    ...
    if (nextDeliveredIds.length > deliveredIds.length) playSfx('deliver');
    else if (nextOnboardId && !onboardId) playSfx('select');
    else if (stationUsed) playSfx('cashout');
    else playSfx('move');
  ...
  }, [completeLevel, deliveredIds, failRun, finished, introDone, level, onboardId, player, registerConstraintViolation, routeBudget, wallSet]);
  ...
  const handleKeyDown = useCallback((event) => {
    if (!introDone || finished || failed || !level) return;
    const action = passengerKeyAction(event.key);
    if (!action) return;
    event.preventDefault();
    if (action.action === 'move') move(action.direction);
    else if (action.action === 'replan') registerReplan();
  }, [failed, finished, introDone, level, move, registerReplan]);
  ...
  for (let y = 0; y < level.rows; y += 1) {
    for (let x = 0; x < level.cols; x += 1) {
      ...
      const destination = level.passengers.find((item) => (
        item.destination.x === x && item.destination.y === y
      ));
      const destinationDelivered = Boolean(destination && deliveredIds.includes(destination.id));
      const cellClasses = [
      ...
      cells.push(
        ...
        >
          {destination && !isWall && (
            <span
              className={`passenger-route-task__destination ${destinationDelivered ? 'passenger-route-task__destination--delivered' : ''}`}
              aria-label={`Destino ${destination.id}`}
              title={destinationDelivered ? `Pasajero ${destination.id} entregado` : `Entregar pasajero ${destination.id}`}
              style={destinationDelivered ? undefined : { borderColor: destination.color, color: destination.color }}
            >
              {destinationDelivered ? '✓' : '⚑'} {destination.id}
            </span>
          )}
  ...
  return (
    <div className="passenger-route-task" tabIndex={0} onKeyDown={handleKeyDown} aria-label={`${t('Central de movilidad', 'Mobility center')} — ${t('Usa las flechas para mover el vehículo y R para registrar un replanteo.', 'Use the arrow keys to move the vehicle and R to register a replan.')}`}>
      ...
      <section className="passenger-route-task__mission" aria-label={t('Misión de movilidad', 'Mobility mission')}>
        ...
        <dl>
          <div><dt>{t('Reserva de la ruta óptima', 'Optimal route reserve')}</dt><dd>{authoredSolution.remainingBudget ?? '—'} {t('de energía', 'of energy')}</dd></div>
        ...
        >
          {!introDone && (
            <div className="game-micro-intro__backdrop" data-testid="game-micro-intro-backdrop">
              <GameMicroIntro gameId="passenger_routes" t={t} onDone={handleIntroDone} />
            </div>
          )}
          {cells}
          <span
            className={`passenger-route-task__token ${onboardId ? 'passenger-route-task__token--loaded' : ''}`}
            ...
            style={{
              ...
              transform: `translate(${player.x * (metrics.cellSize + metrics.gap) + metrics.padding}px, ${player.y * (metrics.cellSize + metrics.gap) + metrics.padding}px)`,
            }}
            aria-label={onboardId ? `Vehículo con pasajero ${onboardId}` : 'Vehículo disponible'}
          >
            🚐{onboardId ? <span key={onboardId} className="passenger-route-task__onboard-badge">{onboardId}</span> : null}
          </span>
          {circuitToast && <span className="passenger-route-task__circuit-toast" role="status">✓ {circuitToast}</span>}
        </div>
      ...
      <div className="passenger-route-task__footer">
        ...
        <small className="passenger-route-task__keyboard-hint">{t(GAME_KEYBOARD.passenger.hintEs, GAME_KEYBOARD.passenger.hintEn)}</small>
      </div>
    </div>
  );
}
