...
import { GAME_KEYBOARD, teamKeyAction } from './gameKeyboard.js';
  ...
  return (
    <section className="team-coordination-task__rpg" aria-label={t('Comando de crisis RPG', 'Crisis command RPG')}>
      <div className="team-coordination-task__rpg-stage">
        {!introDone && (
          <div className="game-micro-intro__backdrop" data-testid="game-micro-intro-backdrop">
            <GameMicroIntro gameId="team_coordination" t={t} onDone={onIntroDone} />
          </div>
        )}
        <div className="team-coordination-task__rpg-topline">
          <span>{t('RPG táctico', 'Tactical RPG')}</span>
          <strong>{scene.location ?? scene.locationEn}</strong>
          <small>{t('Turno', 'Turn')} {currentIndex + 1} {t('de', 'of')} {scenarioCount}</small>
          <GamePips step={currentIndex} total={scenarioCount} className="game-pips--compact" />
        </div>
        <div className="team-coordination-task__party" aria-label={t('Escuadrón', 'Squad')}>
          ...
          {TEAM_PARTY.map((member) => (
            <div key={member.name} className={member.name === scene.speaker ? 'active' : ''} aria-label={`${member.name}, ${member.role ?? member.roleEn}, ${memberEffects[member.name]}`}>
              <span aria-hidden="true">{member.portrait}</span>
              <small><b>{member.name}</b><br />{member.role ?? member.roleEn}<em>{memberEffects[member.name]}</em></small>
            </div>
          ))}
        </div>
        <div key={`dialogue-${scenario.id}`} className="team-coordination-task__dialogue team-coordination-task__dialogue--enter">
          <span className="team-coordination-task__portrait" aria-hidden="true">{scene.portrait}</span>
          <div>
            <span>{scene.act ?? scene.actEn}</span>
            <strong>{scene.speaker} · {scene.role ?? scene.roleEn}</strong>
            <p>{scene.narration ?? scene.narrationEn}</p>
          </div>
        </div>
      </div>
      <div className="team-coordination-task__scenario">
        ...
        <h4>{scenario.title ?? scenario.titleEn}</h4>
        <p>{scenario.prompt ?? scenario.promptEn}</p>
        <div className="team-coordination-task__options" role="group" aria-label={t('Opciones de intervención', 'Intervention options')}>
          {scenario.options.map((option, optionIndex) => (
            <button
              ...
              type="button"
              style={{ '--opt-i': optionIndex }}
              className={`team-coordination-task__option team-coordination-task__option--enter ${selectedOption?.id === option.id ? 'team-coordination-task__option--selected' : ''}`}
            ...
            >
              ...
              <strong>{option.label ?? option.labelEn}</strong>
            </button>
          ))}
        </div>
        {selectedOption && (
          <div className="team-coordination-task__turn-effect team-coordination-task__turn-effect--enter" role="status">
            <strong>{t('Consecuencia de turno', 'Turn consequence')}</strong>
            <p>{selectedOption.why ?? selectedOption.whyEn}</p>
          </div>
        )}
      </div>
    </section>
...
function TeamCoordinationInner({ emit, trialCount, onComplete, practice = false }) {
  const { t } = useLanguage();
  ...
  useEffect(() => {
    if (!introDone || !scenario || finished || shownScenariosRef.current.has(scenario.id)) return;
  ...
  }, [currentIndex, finished, introDone, responses.length, scenario, scenarios.length]);

  const handleOptionSelect = useCallback((option) => {
    if (!introDone || !scenario || selectedOption || finished) return;
    playSfx('select');
  ...
  }, [currentIndex, finished, introDone, responses, scenario, scenarios.length, selectedOption]);
  ...
  const advance = useCallback(() => {
    ...
    onCompleteRef.current?.({
      ...
      ...(practice ? markPracticeSummary('team_coordination', finalAggregate) : finalAggregate),
    });
  }, [currentIndex, practice, scenario, scenarios.length, selectedOption]);

  const handleKeyDown = useCallback((event) => {
    if (!introDone || !scenario || finished) return;
    const action = teamKeyAction(event.key, scenario.options.length);
    if (!action) return;
    event.preventDefault();
    if (action.action === 'choose' && !selectedOption) {
      handleOptionSelect(scenario.options[action.optionIndex]);
    } else if (action.action === 'advance' && selectedOption) {
      advance();
    }
  }, [advance, finished, handleOptionSelect, introDone, scenario, selectedOption]);
  ...
  if (finished) {
    ...
    return (
      <div className="team-coordination-task team-coordination-task--finished" data-testid="team-coordination-finished">
        ...
        <dl>
          <div><dt>{t('Liderazgo (estructurado)', 'Structured leadership')}</dt><dd>{pct(finalAggregate.leadershipScore)}</dd></div>
          <div><dt>{t('Comunicación (estructurada)', 'Structured communication')}</dt><dd>{pct(finalAggregate.communicationScore)}</dd></div>
          <div><dt>{t('Adaptabilidad (contexto)', 'Adaptability (context)')}</dt><dd>{pct(finalAggregate.adaptabilityScore)}</dd></div>
        </dl>
      </div>
    );
  }
  ...
  return (
    <div className="team-coordination-task" data-testid="team-task-root" tabIndex={0} onKeyDown={handleKeyDown} aria-label={`${t('Operación Faro', 'Faro Operation')} — ${t('Usa 1–4 o A–D para elegir y Enter para continuar.', 'Use 1–4 or A–D to choose and Enter to continue.')}`}>
      ...
      <div className="team-coordination-task__footer">
        ...
        <small className="team-coordination-task__keyboard-hint">{t(GAME_KEYBOARD.team.hintEs, GAME_KEYBOARD.team.hintEn)}</small>
