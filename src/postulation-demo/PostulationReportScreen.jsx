...
function WorkbookTalentCard({ construct, t }) {
  return (
    <article className="postulation-demo__talent-card">
      ...
      <div>
        <span className="postulation-demo__provisional-tag">{t('Demo provisional', 'Provisional demo')}</span>
  ...
  return (
    <section className="postulation-demo__report-screen" aria-labelledby="postulation-report-title">
      <div className="postulation-demo__report-hero">
        <div>
          <span className="postulation-demo__eyebrow">{isFixture ? t('Reporte de demostración', 'Demonstration report') : t('Reporte de sesión', 'Session report')}</span>
          <h1 id="postulation-report-title">{isFixture ? t('Reporte de demostración listo para revisión humana', 'Demonstration report ready for human review') : t('Reporte de sesión listo para revisión humana', 'Session report ready for human review')}</h1>
          <p>
            {t('Completaste {completed} de {total} juegos. KRUMM generó una lectura observacional con indicadores de demo, límites explícitos y artefactos locales verificados, sin decisión automatizada.', 'You completed {completed} of {total} games. KRUMM generated an observational reading with demo indicators, explicit limits, and verified local artifacts, with no automated decision.', { completed: completedCount, total: totalCount })}
          </p>
        </div>
          ...
          <div className="postulation-demo__report-section-head">
            <div>
              ...
              <p>{completeWorkbookCoverage
                ? t('Cobertura de tareas en demo: los ocho constructos tienen score provisional y confianza por constructo.', 'Demo task coverage: the eight constructs have provisional score and per-construct confidence.')
