...
export default function PostulationGameStage({
  ...
  const [sfxOn, setSfxOn] = useState(() => getGameSfxEnabled());
      ...
      <div className="postulation-demo__game-body">
        ...
        <div className="postulation-demo__game-hud-corner">
          ...
          <button
            ...
            aria-pressed={sfxOn}
            aria-label={sfxOn ? t('Efectos de sonido: activados', 'Sound effects: on') : t('Efectos de sonido: desactivados', 'Sound effects: off')}
            title={sfxOn ? t('Desactivar efectos de sonido', 'Turn sound effects off') : t('Activar efectos de sonido', 'Turn sound effects on')}
            onClick={toggleSfx}
          >
            {sfxOn ? '🔊' : '🔇'}
          </button>
