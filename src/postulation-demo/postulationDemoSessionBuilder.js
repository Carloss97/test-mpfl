...
function buildAggregateEdgeAIResult({ gameSummary, qualitySummary, extraCaveats = [] }) {
  ...
  return {
    ...
    channels: {
      ...
      recoveryContext: channel(mean([taskPerformance, completion], 0), ['demo_completion']),
      ...
      tensionSignal: channel(0.5, ['not_inferred_without_signal_context', 'facial_tension_not_stress_inference']),
      fatigueIndex: channel(0.5, ['not_inferred_without_signal_context']),
      // Alias de compatibilidad (claves viejas) apuntan al canal renombrado.
      adaptiveResilience: channel(mean([taskPerformance, completion], 0), ['demo_completion']),
      stressResponse: channel(0.5, ['not_inferred_without_signal_context', 'facial_tension_not_stress_inference']),
    },
    caveats: [...new Set([
...
export function buildPostulationDemoArtifacts({
  ...
  const modeCaveats = batteryMode === POSTULATION_DEMO_BATTERY_MODES.ORIGINAL_GAMES
    ...
    : [];
  const { practice: practiceDemoBlocks } = splitBlocksByPractice(completedDemo?.blocks ?? []);
  // Practice/preview runs are excluded from the evaluative profile: only the
  // evaluative blocks may feed the game summary, feature vector and talent
  // framework. Practice summaries are still surfaced (aggregate-only).
  const evaluativeCompletedDemo = {
    ...completedDemo,
    blocks: splitBlocksByPractice(completedDemo?.blocks ?? []).evaluative,
  };
  const blocks = normalizeCompletedBlocks(evaluativeCompletedDemo);
  const practiceSummary = summarizePractice(practiceDemoBlocks);
