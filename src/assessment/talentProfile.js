...
function channel(edgeAI = {}, name, fallback = 50, alias = null) {
  let value = Number(edgeAI.channels?.[name]?.score);
  if (!Number.isFinite(value) && alias != null) value = Number(edgeAI.channels?.[alias]?.score);
...
function buildDimensions(session) {
  ...
  return {
    ...
    adaptability: dimension({
      id: 'adaptability',
      rawScore: mean([channel(edgeAI, 'recoveryContext', 50, 'adaptiveResilience'), difficultyDirections.includes('up') ? 0.85 : difficultyDirections.includes('down') ? 0.35 : 0.6, accuracy]),
      confidence: quality,
      caveats,
      evidence: [`recoveryContext ${Math.round(channel(edgeAI, 'recoveryContext', 50, 'adaptiveResilience') * 100)}%`, `difficulty directions ${difficultyDirections.join(',') || 'hold'}`],
    }),
    ...
    regulationUnderLoad: dimension({
      ...
      rawScore: mean([inverseRate(channel(edgeAI, 'cognitiveLoad')), inverseRate(channel(edgeAI, 'tensionSignal', 50, 'stressResponse')), inverseRate(channel(edgeAI, 'fatigueIndex')), clamp(1 + postureDelta)]),
      confidence: quality,
      caveats,
      evidence: [`cognitiveLoad ${Math.round(channel(edgeAI, 'cognitiveLoad') * 100)}%`, `tensionSignal ${Math.round(channel(edgeAI, 'tensionSignal', 50, 'stressResponse') * 100)}%`, `reactionPostureDelta ${postureDelta.toFixed(3)}`],
    }),
  };
}
...
export function buildTalentProfile({ assessmentSession }) {
  ...
  return {
    schemaVersion: 'krumm_talent_profile_v2',
