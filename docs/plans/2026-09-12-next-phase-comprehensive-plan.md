...
**[x] Implementada y verificada live 2026-09-13 (KRU-118, commits a0a23ae→4f3d166):** `src/analytics/` (analytics.js + ConsentBanner) — posthog-js **variante `dist/module.no-external.js`** (docs PostHog: sin scripts externos → session replay/surveys/site-apps imposibles por construcción, sin expandir `script-src`), import dinámico (solo tras consentir; DCE lo elimina de builds sin key), opt-in vía banner + `cookie_consent` (1 año, forma de la política de privacidad; banner solo aparece si hay key de build), rutas excluidas `/postulaciones*` y `/dev*` (sin pageview/autocapture; solo whitelist funnel: invite_opened, consent_accepted, game_N_completed, report_viewed), scrubber de doble barrera (keys prohibidas: biometría/PII/tokens), `person_profiles:'never'`, sin identify. **Build:** `VITE_POSTHOG_API` (secret `POSTHOG_PROJECT_API_KEY`) + `VITE_POSTHOG_HOST=https://us.i.posthog.com` (ingestión US) en cd.yml. **CSP m6** (stage+prod, solo connect-src: us.i/us/us-assets posthog). **Pageview vía `capture('$pageview')`** (page() no fluía en 1.4xx tras init inmediato). **Evidencia live:** smoke `scripts/smoke-f1-posthog-2026-09-13.mjs` PASS — payload gzip real decodificado: $pageview landing (camino banner y camino visitor-recurrente), invite_opened en /postulaciones, CERO pageviews en ruta excluida, 0 errores CSP. Nota E2E: posthog-js 2025+ filtra bots por UA + `navigator.webdriver` (Playwright necesita webdriver=false + UA normal). Eventos: funnel completo en frontend + `invite_received` server-side (F.2: Lambda → /capture/ con key de proyecto, no python ni key personal).
...
**[x] Implementado 2026-09-13:** (1) `invite_received` **server-side** (Lambda backend → `POST us.posthog.com/capture/`, key de proyecto vía parámetro CFN `PosthogKey` NoEcho, best-effort 2s timeout, `distinct_id` fijo `krumm-backend`, solo prop `language` — sin PII; dispara solo si SES `sent=true`); (2) `duration_s` en `game_N_completed` (reloj de partida en PostulationGameStage, se reinicia por juego → p50/p95 de tiempo por juego en PostHog); (3) **NPS 1-10 opcional** post-reporte en `PostulationReportScreen` (evento `nps_submitted` + whitelist en analytics.js; solo el score viaja; oculto en fixture/demo sintética); (4) **`docs/ops/metrics.md`** (catálogo de eventos, 6 insights a configurar en el dashboard, rutina semanal de screenshots al kanban heartbeat `t_88701e67`, ops de credenciales/region). Tests: backend 10 nuevos (helper + handler), cliente 3 nuevos (NPS) + whitelist. Retención recruiter = insight sobre `recruiter_login` ya existente (sin código nuevo). ⚠️ Configuración de los insights 1-6 en el dashboard = acción del usuario (cuenta PostHog).

### F.3 FinOps mensual
**[x] Implementado 2026-09-13:** `scripts/cost-projection.sh` (presupuesto AWS por API + CloudWatch uso medido: Lambda invocaciones/duración, DynamoDB WCU/RCU ×3 tablas, CloudFront GB ×2 dists, SES Send/Delivery, APIGW por ApiId → costo analítico lista-us-east-1 → costo por evaluación completa) + `docs/ops/finops.md`. Primera corrida (1-13 sep): infra staging **≈$0.034/mes** (CF 0.35 GB, Lambda 154 inv, DDB 92W/262R, APIGW 153 req); **$0.00078 por evaluación** (~$0.78 cada 1,000 evaluaciones); presupuesto `krumm-staging-monthly-billing` $25/mes forecast **$0.27**. Notas: CE ≈$0 en cuenta nueva (top-5 en consola Billing); alarmas 80/100% del presupuesto = verificar en consola (SDK viejo no las ve); "$29.62 del presupuesto de cuenta" ≠ stack KRUMM (verificar en consola).
