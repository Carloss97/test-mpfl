...
- H4.6 Audit visual unificado (desktop + móvil, ES/EN, overflow 0, contraste) + deploy + verificación browser. **AUDIT COMPLETADO (t_be89dafb, 2026-09-07)**: 19 vistas ES/EN en 1280×720 + 390×844, 0 overflow, 0 console errors, contraste AA ≥5.7:1 en todos los pares chrome; fix del footer laser (hallazgo H4.5, recorte preexistente) con spec guardián; hallazgo nuevo hero (cards flotantes ocultaban chip "SCORE PROVISIONAL" + nota del mock) fijado y documentado — **superado por el port de marca en curso** (plan `2026-09-07-landing-brand-port-plan.md`: elimina mock/stat cards). **Deploy + push pendientes de decisión del usuario** (colisión con el deploy incluido en el port de marca) — ver `docs/qa/h46-visual-audit/h46-visual-audit.md`.
...
- Reglas de uso para futuras experiencias (los agentes leen design-system.md + tokens antes de UI nueva) — se documenta en AGENTS.md al cerrar H4. **PENDIENTE DE APROBACIÓN (2026-09-07):** el texto propuesto está en el comentario de t_be89dafb (sección "## Design system (H5) — reglas de UI"); el patch a AGENTS.md (archivo protegido) expiró por approval timeout — aplicar en sesión interactiva. Nota: el port de marca v2 (Archivo/Manrope, paleta oficial) actualizará design-system.md/tokens; la sección H5 sigue siendo válida (fuente de verdad = design-system.md + tokens --k-*).
