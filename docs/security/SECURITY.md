...
- 2026-09-14: **G.3 (KRU-138) — rate limiting API + WAF CloudFront + baseline ZAP.** (a) Rate limit 10 req/min/IP en `*/invitations*` + `*/sessions*` (ambos stages): token por minuto en la Lambda con tabla DDB `krumm-staging-rate-limit` (TTL 3 min) — E2E verificado: 12× GET /staging/sessions → 10×200 + 2×429 (`retry-after`). **Desviación del plan**: la asociación WAF↔API Gateway la rechaza la API WAF 2026 en 6 formatos de ARN → rate limit en Lambda (robusto, documentado). (b) WAF CloudFront `krumm-cf-waf` (`AWSManagedRulesCommonRuleSet` CRS + `AWSManagedRulesKnownBadInputsRuleSet`) asociado a las dos distribuciones **vía ARN** (el ID falla: "not accessible by requester"); verificación de evaluación/bloqueo pendiente (métricas WAF con latencia ~15 min). (c) Baseline OWASP ZAP 2.17 (/, /portal, /candidato): **0 critical/high** — 1 Medium aceptado (CSP `style-src 'unsafe-inline'`, by design), 1 Low aceptado (header `Server` AWS). Reportes: `docs/security/g3-zap-baseline-2026-09-14/`. (d) gitleaks: `--redact` activo por defecto en `gitleaks-action@v3` (verificado, sin cambios). Detalles, evidencia y lecciones de la API WAF/CFN/ZAP 2026: **`docs/security/security-waf-zap.md`**.
