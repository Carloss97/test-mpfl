...
- 2026-09-13: **CSP m4** — el `connect-src` (m3) no incluía el origen de Cognito, así que el navegador bloqueaba el fetch del token exchange (PKCE) del login de empresa → `auth_exchange_failed` en `stage.krumm.cl/empresa/acceso`. Fix: RHP `krumm-staging-rhp-m4` = m3 + `https://krumm-staging-recruiters.auth.us-east-1.amazoncognito.com` en `connect-src`, aplicada a las DOS distribuciones (`E2OPPVGDO8R75S` stage + `EDQ39PDNI931R` prod), m3 eliminada. Evidencia: E2E Playwright `scripts/smoke-a2-login-2026-09-13.mjs` (login real con cuenta QA). Nota: cuando se cree el hosted UI de prod, agregar su origen + m5.
- 2026-09-13: **CSP m6 (RHP `krumm-staging-rhp-m6`, `33553c61`)** — F.1 (KRU-118, PostHog): `connect-src` += `us.i.posthog.com` (ingestión /e/), `us.posthog.com` (flags) y `us-assets.i.posthog.com` (fetch de remote config). **Sin tocar `script-src`**: posthog-js se importa en la variante `dist/module.no-external.js` (docs PostHog) → no carga scripts externos; session replay/surveys/site-apps imposibles por construcción (relevante: el proyecto PostHog tiene sessionRecording + console-log activados server-side; no-external los hace inaplicables en el cliente). Conmutadas ambas distribuciones (stage `E2OPPVGDO8R75S` + prod `EDQ39PDNI931R`), m5/m4/m3 eliminadas, m1 orphan no tocada. Evidencia: smoke live `scripts/smoke-f1-posthog-2026-09-13.mjs` (payload real gzip decodificado: $pageview landing + invite_opened + exclusión /postulaciones; 0 errores CSP). Nota ops: posthog-js 2025+ filtra bots por UA + `navigator.webdriver` (los E2E headless necesitan webdriver=false + UA normal).
- 2026-09-14: **G.3 (KRU-138) — rate limiting API + WAF CloudFront + baseline ZAP.** (a) Rate limit 10 req/min/IP en `*/invitations*` + `*/sessions*` (ambos stages): token por minuto en la Lambda con tabla DDB `krumm-staging-rate-limit` (TTL 3 min) — E2E verificado: 12× GET /staging/sessions → 10×200 + 2×429 (`retry-after`). **Desviación del plan**: la asociación WAF↔API Gateway la rechaza la API WAF 2026 en 6 formatos de ARN → rate limit en Lambda (robusto, documentado). (b) WAF CloudFront `krumm-cf-waf` (`AWSManagedRulesCommonRuleSet` CRS + `AWSManagedRulesKnownBadInputsRuleSet`) asociado a las dos distribuciones **vía ARN** (el ID falla: "not accessible by requester"); 03:25 UTC: el edge aún no lo evalúa (0 métricas WAF tras ~35 min + probes 200 — coherente con la migración service-side WAF 2026) → verificar/crear por consola (CloudFront → distribución → pestaña Web ACL). (c) Baseline OWASP ZAP 2.17 (/, /portal, /candidato): **0 critical/high** — 1 Medium aceptado (CSP `style-src 'unsafe-inline'`, by design), 1 Low aceptado (header `Server` AWS). Reportes: `docs/security/g3-zap-baseline-2026-09-14/`. (d) gitleaks: `--redact` activo por defecto en `gitleaks-action@v3` (verificado, sin cambios). Detalles, evidencia y lecciones de la API WAF/CFN/ZAP 2026: **`docs/security/security-waf-zap.md`**.
