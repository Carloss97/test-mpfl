...
| 5 | VisualSearch (`visual_search`) | stable_dg | [2026-09-12](2026-09-12-visual-search.md) | ✅ PASS (3 P1 + 5 P2 + 5 P3 cerrados: timeout por trial 10 s, geometría bloqueada initialSizeRef, doc módulo R-6 (5.º/5.º stable_dg), contrato session builder + Puntaje en card de reporte, localización real del pointer, fit canvas 240 px + tile ≥44, rng por sesión, ITI tracked + jitter + feedback chip; 3 P3 abiertos sistemáticos + 2 decisionados) | 0 | 0 | 0 | 3 | ✅ |
| 6 | BalloonRisk (`balloon_risk`) | original | [2026-09-12](2026-09-12-balloon-risk.md) | ✅ PASS (2 P1 + 1 P2 cerrados: riskEfficiency clamp 816>800 (óptimo invalidaba bloque), input lock post-settle (doble respuesta en FX 900/500 ms), SFX toggle 42→44 px; 6 P3 decisionados/abiertos sistemáticos; primer juego original jugado en vivo en batería) | 0 | 0 | 0 | 6 | ✅ |
...
- Docs de módulo: los 7 juegos original tienen doc en `docs/design/modulos/`; los 5 de stable_dg NO la tenían → P1 sistemático para 2-5. **CIERRADO (B.5, 2026-09-12):** 5/5 docs de stable_dg creadas (simple_rt pendiente de re-verificación en B.1 — su doc `simple_rt.md` sigue abierta en fila 1; precision/go_nogo/color_interference/visual_search done).
...
- **Clases de feedback scopadas por tema (B.5, cerrado en VisualSearch):** `.trial-feedback` solo estaba estilizada bajo `.simple-rt-task` (tema oscuro de App) → en batería renderizaba sin centrado. Para B.6–B.12 (original): verificar que las clases del juego estén estilizadas en contexto postulation (`postulationDemo.css`) y que el feedback en juego sea visible en el smoke (screenshot + vision).
- **Geometría con el tamaño renderizado (B.5, cerrado en VisualSearch):** la rejilla se calculaba con `safeWidth = max(260, w)` mientras el canvas se pintaba a `w=240` (piso del stage) → tiles recortados por `overflow:hidden` en setSize 20. Para B.6–B.12 (juegos canvas-based): la geometría (celdas/grid) debe calcularse con el tamaño REAL que se pinta; test de fit en el piso del stage (240×280) y smoke con asert de elementos-completos-en-canvas por pantalla.
- **Contrato de keys del summary (B.5, mismo bug GNP-P2-1/CIP-P2-1):** `summarizeCompletedBlocks`/`getDefaultGameMetrics` piden `completedTrialCount`, `meanReactionTimeMs` y `score ?? meanScore` en el summary del bloque — si faltan, la card del reporte muestra "Tiempo"/"Puntaje" ausentes y la media de batería se sesga. Para cada juego B.6–B.12: verificar que el summary del flujo VIVO (no solo el fixture) lleva esas 3 keys.
- **Pantalla final del último juego = reporte (B.5):** en VS (juego 4 de 4) el `onComplete` monta el reporte en el mismo batch → el screenshot tras el click del panel final captura el reporte, no el feedback. Para evidencia del último panel, capturar ANTES del click (patrón `smoke-b5-visual-search-visuals-2026-09-12.mjs`).
- **Smoke de batería original: el juego previo se completa, no se salta (B.6, primer juego original en vivo):** Balloon es juego 2 de 7 → el smoke debe resolver Laser completo (3 niveles) con las `solutionPlacements` embebidas en `laserPuzzleTelemetry.js` (testid real `laser-cell-<x>,<y>` con COMA, p. ej. `laser-cell-7,0`; no `laser-cell-7-0`), origen→destino por par + "Comprobar ruta" + wait ~3.8 s (clear 1500 + interstitial 1400). Patrón: `completeLaser`/`skipMicroIntro` de `smoke-t_f40921bf-stage-mobile.mjs` (reusado en `smoke-b6-balloon-risk-2026-09-12.mjs`). Para B.7+ (juegos 3-7) el smoke heredará: laser + balloon + juegos intermedios completados.
- **Asserts de texto del reporte: `text-transform: uppercase` (B.6):** el reporte fixture/vivo renderiza etiquetas en mayúsculas (feedback label, tag "Score provisional") → `innerText` devuelve el texto TRANSFORMADO y los asserts case-sensitive fallan falsamente. Comparar en minúsculas (patrón `lower.includes(...)` del smoke B.6) o usar `textContent`.
- **WIP de re-dispatch: verificar el smoke script del run previo, no confiarlo (B.6):** el run 116 (failover NIM) dejó código+tests+doc GREEN pero un smoke script con testids incorrectos y flujo imposible; este run lo reescribió. En re-dispatches de FASE B: correr/leer el smoke WIP antes de citarlo como evidencia.
