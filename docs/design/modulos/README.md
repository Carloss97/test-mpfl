...
| Desactivación de Secuencias / Bomb (`bomb_defusal`) | `bomb_defusal.md` | **implementado (EXP-BOMB-001, spec v1.1.0, cadena B1–B6 2026-09-08)** — 9° constructo `proceduralWorkingMemory` (experimental, `descriptive_only`, sin score compuesto §12.1); 6° juego de la batería `original` | `src/tasks/original-games/bomb/` (motor, panel, telemetría §11–14, feedback), `originalGameFeatureVector.js` (12 features `bomb.*`, defs 2.2.0), `originalGameTalentMapping.js` (9° constructo) |
...
> **Proceso para nuevas experiencias (2026-09-06):** ver `docs/plans/2026-09-06-plan-corto-plazo-flujos-y-documentacion.md` §2 (spec 1-pager → doc módulo → TDD §16 → gates → smoke → sync). Sin spec no se implementa (regla R-6).
